...
static int              mem_read( char *b, unsigned bsize, struct Globals *pG );
static void __fastcall  flush_outbuf( unsigned w, unsigned bytes, struct Globals *pG );

// Initialize the bit string routines. zipfile :: Output zip file, NULL for
//   in-memory compression.
void
bi_init( HANDLE zipfile, struct Globals *pG )
{
  pG->zfile = zipfile;
  pG->bi_buf = 0;
  pG->bi_valid = 0;
...
//   bits.
void __fastcall
send_bits( int value, int length, struct Globals *pG )
{
#ifdef DEBUGx
...
flush_outbuf( unsigned w, unsigned bytes, struct Globals *pG )
{
...
void __fastcall
copy_block( char *block, unsigned len, int header, struct Globals *pG )
{
  ...
  else if ( pG->out_offset + len > pG->out_size )
  {
...
ulg
memcompress( char *tgt, ulg tgtsize, char *src, ulg srcsize, struct Globals *pG )
{
  ush att = ( ush ) UNKNOWN;
  ush flags = 0;
...
static int
mem_read( char *b, unsigned bsize, struct Globals *pG )
{
