...
type
  ...
  protected
	...
	procedure AfterCallback(Sender: TObject; var stop: boolean);
...
procedure TZipThread.AfterCallback(Sender: TObject; var stop: boolean);
begin
	if Terminated then
		stop := true;
end;
...
begin
	...
	fSpecs := Specs;
	fMemo := Memo;
	zip := {TZipWorker.Create;//}TZipMaster.Create(nil);	// no owner, so must free
  zip.Active := false;
  zip.NotMainThread := true;
//  zip.NotMainTask := true;
	OnTerminate := Finished;
end;

procedure TZipThread.AfterConstruction;
begin
	zip.OnCheckTerminate := AfterCallback;
//	zip.OnAfterCallback := AfterCallback;
