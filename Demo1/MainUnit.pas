...
type
  Int64 = Comp;
// 64 bit integers are supported differently by Delphi 2 and 3
{$ENDIF}
...
type
  TMainform = class (TForm)
    ...
    procedure ZipMaster1Message(Sender: TObject; ErrCode: Integer;
      Message: String);
//        Procedure ZipMaster1Progress1(Sender: TObject; ProgrType: ProgressType; FileName: String; FileSize: TProgressSize);
    ...
    procedure StringGrid1GetCellFormat(Sender: TObject;
      Col, Row: LongInt; State: TGridDrawState;
    ...
    procedure ZipMaster1ProgressDetails(Sender: TObject;
  ...
  PUBLIC
    ...
    function AskDirDialog(const FormHandle: HWND;
      var DirPath: String): Boolean;
  PROTECTED
    FNewCount: Integer;
  end;
...
begin
  ...
  begin
    ...
    RowCount := 2;
    { first row is fixed, and used for column headers }
    Cells[0, 0] := 'File Name';
...
begin
  ...
  begin
    Ans := MessageDlg('Are you sure you want to delete: ' +
      ZipMaster1.ZipFilename + '?', mtConfirmation,
      [mbYes, mbNo], 0) = mrYes;
    if Ans then
    begin
...
begin
    ...
    begin
      ...
      begin
        ...
        if (SelRow > 0) and (SelRow <> RowCount - 1) then
          ZipMaster1.FSpecArgs.Add(Cells[5, SelRow] +
            Cells[0, SelRow]);
      end;
  ...
  ZipMaster1Message(self, 0, 'Beginning Extract from ' +
...
begin
  ...
  begin
    ...
    if AddForm.AtribOnlyCB.Checked then
      AddOptions := AddOptions + [AddArchiveOnly];
    { we want changed only }
...
begin
  ...
  MsgForm.Show;
  { Put this message into the message form's memo }
  ZipMaster1Message(self, 0, 'Beginning delete from ' +
    ZipMaster1.ZipFileName);
  ...
  if ZipMaster1.SuccessCnt = 1 then
    ...
    IsOne := 's were';
  ShowMessage(IntToStr(ZipMaster1.SuccessCnt) + ' file' +
    IsOne + ' deleted');
end;
...
begin
  ...
  begin
    if SuccessCnt = DirOnlyCount + Count then
      ShowMessage('All ' + IntToStr(DirOnlyCount + Count) +
        ' files tested OK')
    else
      ShowMessage('ERROR: ' + IntToStr(DirOnlyCount +
        Count - SuccessCnt) +
...
begin
  ...
  ShowMessage(ZipMaster1.FullVersionString + #10 +
    ZipMaster1.ZipDllPath + #10 + ZipMaster1.UnzDllPath);
end;
...
procedure TMainform.ZipMaster1Message(Sender: TObject;
  ErrCode: Integer; Message: String);
begin
...
(*
Procedure TMainform.ZipMaster1Progress1(Sender: TObject; ProgrType: ProgressType; Filename: String; FileSize: TProgressSize);
Var
    Step: Integer;
Begin
    Case ProgrType Of
...
begin
  ...
  if ZipMaster1.ZipFilename = '' then
    ...
    ZipFName.Caption :=
      MinimizeName(ZipMaster1.ZipFilename, ZipFName.Canvas, ZipFName.Width);
...
begin
  ...
  begin
    ...
    if TotUnComp <> 0 then
      Cells[4, RowCount - 1] :=
        IntToStr(Round((1 - (TotComp / TotUnComp)) * 100)) + '% '
    else
...
function TMainform.AskDirDialog(const FormHandle: HWND;
  var DirPath: String): Boolean;
var
  pidl: PItemIDList;
...
function TMainform.GetSpecialFolder(aFolder: Integer;
  var Location: String): LongWord;
var
  pidl: PItemIDList;
  hRes: HRESULT;
...
const
  sorts: array[1..6] of TSortStyle =
    (ssAutomatic, ssAlphabetic, ssNumeric, ssDateTime, ssTime, ssCustom);
var
  i: Integer;
  so: TSortOptions;
begin
    ...
    begin
      ...
      begin
        ...
        if UncompressedSize <> 0 then
          Cells[4, i] :=
            IntToStr(Round((1 - (CompressedSize / UnCompressedSize)) *
            100)) + '% '
        else
          Cells[4, i] := '0% ';
...
procedure TMainform.StringGrid1GetCellFormat(Sender: TObject;
  Col, Row: LongInt; State: TGridDrawState; var FormatOptions: TFormatOptions);
begin
...
// New less complicated progress event
// note ProgressBar now have preset max := 100
procedure TMainform.ZipMaster1ProgressDetails(Sender: TObject;
  details: TProgressDetails);
begin
...
//      MsgForm.ProgressBar2.Max := 100;
    ...
    begin
      ...
      MsgForm.StatusBar1.Panels.Items[1].Text :=
        IntToStr(details.TotalCount) + ' files';
    end;
    NewFile:
    begin
