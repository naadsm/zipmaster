...
int
IsFileSystemOldFAT( char *dir, struct Globals *pG )
{
  char  *fp,
        *p;
  DWORD vfnsize;
  ...
  {
    Inform( pG, 0, 0, "zip diagnostic: GetVolumeInformation failed for %s\n", fp );
    return ( false );
  }
...
int
GetFileMode( struct Globals *pG, char *name )
{
  DWORD dwAttr;
  ...
  {                             // RCV Changed: was ...= -1
    Inform( pG, 0, 0, "zip diagnostic: GetFileAttributes failed\n" );
...
// long GetTheFileTime( char *name ) { HANDLE h; FILETIME ft, lft; WORD dh,
//   dl; h = CreateFile( name, GENERIC_READ, FILE_SHARE_READ |
//   FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL ); if (
//   h != INVALID_HANDLE_VALUE ) { GetFileTime( h, NULL, NULL, &
//   ft ); FileTimeToLocalFileTime( &
//   ft, &
//   lft ); FileTimeToDosDateTime( &
//   lft, &
//   dh, &
//   dl ); CloseHandle( h ); return( dh << 16 ) | dl; } else return 0L; }
// void ChangeNameForFAT( char *name ) { static char invalid[] = ":\"<>|\t";
//   // V1.4 Changed was: ":;,=+\"[]<>| \t" char *src, *dst, *next, *ptr, *dot,
//   *start; if ( isalpha( name[0] ) &
//   &
//   (name[1] == ':') ) start = name + 2; else start = name; src = dst = start;
//   if ( *src == '\\' ) src++, dst++; while ( *src ) { for ( next = src; *next &
//   &
//   (*next != '\\'); next++ ); // SLASH for ( ptr = src, dot = NULL; ptr <
//   next; ptr++ ) if ( *ptr == '.' ) { dot = ptr; // remember last dot ptr =
//   '_'; } if ( dot == NULL ) for ( ptr = src; ptr < next; ptr++ ) if ( *ptr
//   == '_' ) dot = ptr; // remember last _ as if it were a dot if ( dot &
//   &
//   (dot > src) &
//   &
//   ((next - dot <= 4) || ((next - src > 8) &
//   &
//   (dot - src > 3))) ) { if ( dot ) *dot = '.'; for ( ptr = src; (ptr < dot) &
//   &
//   ((ptr - src) < 8); ptr++ ) *dst++ = *ptr; for ( ptr = dot; (ptr < next) &
//   &
//   ((ptr - dot) < 4); ptr++ ) *dst++ = *ptr; } else { if ( dot &
//   &
//   (next - src == 1) ) dot = '.'; // special case: "." as a path component
//   for ( ptr = src; (ptr < next) &
//   &
//   ((ptr - src) < 8); ptr++ ) *dst++ = *ptr; } dst++ = *next; // either '/'
//   or 0 if ( *next ) { src = next + 1; if ( *src == 0 ) // handle trailing
//   '/' on dirs ! dst = 0; } else break; } for ( src = start; *src != 0; ++src
//   ) if ( strchr( invalid, *src ) != NULL ) *src = '_'; // V1.4 removed was
//   ... || (*src == ' ') }
char *
GetLongPathEA( void )
{
...
// int IsFileNameValid( char *x ) { WIN32_FIND_DATA fd; HANDLE h =
//   FindFirstFile( x, &
//   fd ); if ( h == INVALID_HANDLE_VALUE ) return false; FindClose( h );
//   return true; }
...
char *
getVolumeLabel( struct Globals *pG, int Drive, ulg *vtime, ulg *vmode, time_t *vutim )
{                               // v1.6017
  char  RootPath[4];
  ulg   fnlen,
        flags;
