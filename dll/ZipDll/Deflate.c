...
static void fill_window( struct Globals * pG );
static ulg deflate_fast( struct Globals * pG );

extern int __fastcall longest_match( IPos cur_match, struct Globals * pG );

#ifdef DEBUG
static void check_match( IPos start, IPos match, int length, struct Globals * pG );
#endif
...
void lm_init( int pack_level, ush * flags, struct Globals * pG )
{
  ...
  pG->lookahead = ( * pG->read_buf ) ( ( char * ) pG->window, j, pG );
...
#if 0
int __fastcall longest_match( IPos cur_match, struct Globals * pG )
{
  ...
  ///* far
  IPos limit = pG->strstart > ( IPos )MAX_DIST ? pG->strstart - ( IPos )MAX_DIST : NIL;
...
{ 
  ...
  {
    Inform( pG, -1, 2, "invalid match - start %d, match %d, length %d", start, match, length );
  }
}
...
static void fill_window( struct Globals * pG )
{
  ...
  do
  {
    ...
    else if ( pG->strstart >= WSIZE + MAX_DIST && pG->sliding )
    {
    ...
    n = ( * pG->read_buf ) ( ( char * ) pG->window + pG->strstart + pG->lookahead, more, pG );
