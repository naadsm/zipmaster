...
/* ===========================================================================
* Update the encryption keys with the next byte of plain text c :: Byte of plain text. */
...
int decrypt( struct Globals * pG )
{
    ...
    if ( pG->P_flag == true )
    {
      ...
      if ( !pG->key )
      {
        ...
        if ( pG->CallerVersion < 160 )
          pG->nopwd = true; // RCV: 1.607 changed
      }
  ...
  else if ( ( pG->key = ( char * ) MALLOC( PWLEN + 1 ) ) == ( char * ) NULL )
    return PK_MEM2;
  ...
  do
  {
    ...
    { // RCV: 1.607
      ...
      if ( pG->CallBackData.actioncode != 8 )
      { // v1.6024
        pG->nopwd = true;
        n1 = 1;
      }
      n = min( ( int )pG->CallBackData.fsize & 0x0F, n1 );
    }
    else
    {
      r = UzpGetPassWrd( pG );
      ...
      if ( r == IZ_PW_ERROR )
      {
        ...
        FREE( pG->key );
        /* No DialogBox, Password too long, memcopy failed */
        pG->key = NULL;
        return PK_MEM2;
      }
      if ( r != IZ_PW_ENTERED )
      {
        /* user replied "Cancel" or entered with NULL string */
        * pG->key = '\0';
        /*   We try the NIL password, ...        */
        n = 0;
        /*   and cancel fetch for this item. */
      }
      if ( !testp( pG, h ) )
        return PK_COOL;
