 ...
 * 1.75 Edited 22 February 2004 R. Peters
 */
...
#define ACTION( cd, pre, id)  (((id)<<16) + ((pre)<<8) + (cd)))
#define T_ACTION( c, p, i ) {Actions _ta; _ta.ident = i; _ta.pre = p; _ta.action = c;}
...
//    char filenameormsg[ 512 ];      /* NOTE: NOT a pointer - data is here */
  union{
    ...
    {
      int zero;
      char* fname;                      // NOTE: is pointer to data
      unsigned args[123];
    };
  };
}
...
typedef struct
{
    ...
    char *fFileAltName;
    char *fPassword;
    //  long fNotUsed[15]; altered 1.7.0.5 by R.Peters - no change of size
    long fNotUsed[ 14 ];
...
typedef struct
{                // Excluded file specs, not implemented yet
    char *fFileSpec;
    long fNotUsed[ 3 ];
}
fExFileData;
