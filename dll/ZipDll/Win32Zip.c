...
static zDIR *
Opendir( const char *n, struct Globals *pG )
{
...
//   occurs.
static char *
readd( zDIR *d, struct Globals *pG )
{
  struct zdirent  *e;
...
Wild( char *w, struct Globals *pG )
{
  ...
  {
    pG->volume_label = 2;
    pG->label = getVolumeLabel( pG, (w != NULL && w[1] == ':') ? to_up( w[0] ) : '\0',
...
int
procname(char *ArgName, bool RecurseDir, struct Globals *pG )
{
  ...
  if ( LSSTAT( GetFullPath( pG, ArgName ), &StatRes ) || (IsShExp( ArgName ) != NULL) )
  {
    ...
    for ( z = pG->zfiles; z; z = z->nxt )
    {
      if ( dosmatch( p, z->zname /* , pG */ ) )
      {
        ...
        if ( pG->verbose )
          Inform( pG, 0, 0, "%scluding %s", z->mark ? "in" : "ex", z->name );
  ...
  if ( !(StatRes.st_mode & S_IFDIR) )
  {
    ...
    if ( pG->verbose )
    {
      Inform( pG, 0, IDIAG, "spot 1: Add file %s to found list", ArgName );
    }
    ...
    if ( (m = newname( ArgName, StatRes.st_size, pG )) != ZEN_OK )
    {
      if ( pG->verbose < 0)
      {
        Inform( pG, 0, IDIAG, "returning %d from procname after newname call", m );
      }
  ...
  else
  {
    ...
    else
    {
      ...
      if ( pG->dirnames && (m = newname( p, 0, pG )) != ZEN_OK )
      {
        ...
        if ( pG->verbose < 0 )
        {
          Inform( pG, 0, IDIAG, "returning %d from procname after 2nd newname call", m );
        }
      ...
      while ( (e = readd( d, pG )) != NULL )
      {
        ...
        if ( strcmp( e, "." ) && strcmp( e, ".." ) )
        {
          ...
          if ( (m = procname( a, RecurseDir, pG )) != ZEN_OK )
          {
            ...
            else if ( ( int ) ( char )(m & 0xFF) == ZEN_MISS )
              Inform( pG, 0, IWARNING, "name not matched: %s", a );
          }
...
char *
ex2in( char *x, int *pdosflag, struct Globals *pG )
{
  ...
  if ( !pG->dosify && pG->use_longname_ea && (t = GetLongPathEA()) != NULL )
  {
    if ( pG->verbose )
    {
      Inform( pG, 0, IDIAG, "replacing short name: %s with long name: %s", x, t );
    }
  ...
  //	const char *pfp;
/*  if ( ((isalpha( x2[0] ) && x2[1] == ':') || (x2[0] == '\\' && x2[1] == '\\')) )
    pfp = x2;
  else
    pfp = GetFullPath( pG, x );
  user_callback( zacNewName, ( int )pfp, 0, n, pG );
*/
  //		user_callback( zacNewName, (int) GetFullPath( pG, x ), 0, n, pG );
  FREE( x2 );
//  if ( /*pG->callbackdata.error_code != pfp &&*/ pG->callbackdata.error_code )
/*  {
    FREE( n );
    if ( (n = ( char * )MALLOC( lstrlen( pG->callbackdata.filenameormsg ) + 1 )) == NULL )
      return NULL;
...
stamp( char *f, ulg d )
{
  ...
  if ( DosDateTimeToFileTime( (( struct DosDate * ) &d)->hi, (( struct DosDate * ) &d)->lo,
                              &LocalFT )
   && LocalFileTimeToFileTime( &LocalFT, &FT ) ) SetFileTime( fHandle, NULL, NULL, &FT );
  CloseHandle( fHandle );
}
...
ulg
filetime( char *f, ulg *a, /*unsigned long*/int *n, ztimbuf *t, struct Globals *pG )
{
...
//  if ( LSSTAT( GetFullPath( pG, name ), &s ) != 0 )
  ...
  if ( n != NULL )
  {
    *n = ( s.st_mode & S_IFMT ) == S_IFREG ? s.st_size > MAXINT ? -3L : (LONG) s.st_size  : -1L;
...
{
  #ifdef
  USE_EF_UX_TIME if ( (z->extra = ( char * )MALLOC( EB_HEADSIZE + EB_UX_MINLEN )) == NULL ) return ZEN_MEM24;
  z->extra[0] = 'U';
  z->extra[1] = 'X';
