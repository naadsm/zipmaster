...
static int find_ecrec( struct Globals * pG, long searchlen );


/* ===========================================================================
*                     Function process_zipfiles() * return PK-type error code */
void process_zipfiles( struct Globals * pG )
{
  ...
  *---------------------------------------------------------------------------*/
  ...
  while ( ( pG->zipfn = do_wild( pG, pG->wildzipfn ) ) != NULL )
  {
    ...
    Trace( ( pG, "after do_seekable, warn=%d   missdir=%d  missfi=%d  losefi=%d  winfi=%d", NumWarnFiles, NumMissDirs,
  ...
  if ( ( NumWinFiles + NumWarnFiles + NumLoseFiles ) == 0 && ( NumMissDirs + NumMissFiles ) == 1
       && lastzipfn != ( char * ) NULL )
       {
         ...
         if ( iswild( pG->wildzipfn ) )
           DLLprintf( "Error: No wildcard match on zipfile: %s", pG->wildzipfn );
         else
         {
           ...
           if ( error == PK_WARN ) /* GRR: make this a switch/case stmt ... */
             ...
             ++NumMissDirs;
           else if ( error == PK_NOZIP ) /* increment again => bug: "1 file had no zipfile directory." */
           /* ++NumMissFiles */
           ;
           else if ( error )
             ++NumLoseFiles;
  ...
  *---------------------------------------------------------------------------*/
  if ( ( NumWinFiles > 1 ) || ( NumWinFiles == 1 && NumMissDirs + NumMissFiles + NumLoseFiles + NumWarnFiles > 0 ) )
    DLLprintf( "files processed OK" );
  if ( NumWarnFiles > 0 )
    DLLprintf( "warnings were given" );
  if ( NumLoseFiles > 0 || NumMissFiles > 0 )
    DLLprintf( "file(s) not found" );
  if ( NumMissDirs == 1 )
    DLLprintf( "zip file was dir" );
  else if ( NumMissDirs > 0 )
    DLLprintf( "many zip file were dirs" );
  if ( NumWinFiles + NumLoseFiles + NumWarnFiles == 0 )
    DLLprintf( "no files found" );
...
static int do_seekable( struct Globals * pG, int lastchance )
{
  ...
  *---------------------------------------------------------------------------*/
  if ( stat( pG->zipfn, & pG->statbuf ) || ( error = S_ISDIR( pG->statbuf.st_mode ) ) != 0 )
  {
    ...
    {
      if ( pG->no_ecrec )
        DLLprintf( "can't find zip file dir" );
      else
        DLLprintf( "can't find either zip file" );
    }
  ...
  *---------------------------------------------------------------------------*/
  ...
  if ( CRC_32_TAB == NULL )
  {
    if ( ( CRC_32_TAB = ( ulg * ) get_crc_table() ) == NULL )
      return PK_MEM2;
  }
  ...
  if ( ( ( ( error_in_archive = find_ecrec( pG, MIN( pG->ziplen, 66000L ) ) ) != 0
       || ( error_in_archive = uz_end_central( pG ) ) > PK_WARN ) ) )
       {
         ...
         if ( maybe_exe )
           DLLprintf( "maybe an EXE file: %s", pG->zipfn );
         if ( lastchance )
  ...
  *---------------------------------------------------------------------------*/
  ...
  if ( !too_weird_to_continue )
  {
    ...
    if ( error )
    {
      DLLprintf( "maybe a pak bug in %s", pG->zipfn );
      error_in_archive = PK_WARN;
    }

    if ( ( pG->extra_bytes = pG->real_ecrec_offset - pG->expect_ecrec_offset ) < 0L )
    {
      DLLprintf( "missing bytes in zipfile" );
    ...
    else if ( pG->extra_bytes > 0 )
    {
      ...
      {
        DLLprintf( "Warning: extra bytes at start of zipfile" );
        error_in_archive = PK_WARN;
      }
    }
    ...
    *-----------------------------------------------------------------------*/
...
#endif
    {
      ...
      DLLprintf( "central dir too long" );
      error_in_archive = PK_ERR;
    }
...
static int find_ecrec( struct Globals * pG, long searchlen )
{
  ...
  diag( pG, "in find_ecrec (end of central dir)" );
...
fail:
  ...
  *---------------------------------------------------------------------------*/
  ...
  Trace( ( pG, "Found end-of-central-dir signature at offset %ld (%.8lXh)\n",
         pG->real_ecrec_offset, pG->real_ecrec_offset ) );
  Trace( ( pG, "    from beginning of file; offset %d (%.4Xh) within block\n",
...
int uz_end_central( struct Globals * pG )
{
  ...
  !pG->qflag ) ) )
  {
    ...
    {
      DLLprintf( "zip file comment too long" );
      error = PK_WARN;
    }
  }
  return error;
...
int process_cdir_file_hdr( struct Globals * pG )
{
  ...
  diag( pG, "in process_cdir_file_hdr()" );
  ...
  *---------------------------------------------------------------------------*/
  ...
  {
    diag( pG, "Error returned by get_cdir_ent call" );
    return error;
  }
  pG->pInfo->hostnum = MIN( pG->crec.version_made_by[1], NUM_HOSTS );
  pG->pInfo->lcflag = 0;

  Trace( ( pG, "Good entry; hostnum of file = %d", pG->pInfo->hostnum ) );
  ...
  diag( pG, "process_cdir_file_hdr returning PK_COOL" );
  return PK_COOL;
}
...
int get_cdir_ent( struct Globals * pG )
{
  ...
  *---------------------------------------------------------------------------*/
  ...
  Trace( ( pG, "End of get_cdir_ent in process.c. Found filename of len %d", pG->crec.filename_length ) );
...
int process_local_file_hdr( struct Globals * pG )
{
  ...
  *---------------------------------------------------------------------------*/
  ...
  Trace( ( pG, "process_local_file_hdr of process.c found fname len of %d", pG->lrec.filename_length ) );
  return PK_COOL;
}
...
unsigned ef_scan_for_izux( uch * ef_buf, unsigned ef_len, ztimbuf * z_utim, ush * z_uidgid )
{
  ...
  while ( ef_len >= EB_HEADSIZE )
  {
    ...
    if ( eb_len > ( ef_len - EB_HEADSIZE ) )
    {
      ...
      TTrace( ( pG, "ef_scan_for_izux: block length %u > rest ef_size %u\n", eb_len, ef_len - EB_HEADSIZE ) );
      break;
    }
    if ( eb_id == EF_IZUNIX && eb_len >= EB_UX_MINLEN )
    {
      ...
      if ( z_utim != NULL )
      {
        ...
        TTrace( ( pG, "  Unix EF access time = %ld\n", z_utim->actime ) );
        TTrace( ( pG, "  Unix EF modif. time = %ld\n", z_utim->modtime ) );
      }
