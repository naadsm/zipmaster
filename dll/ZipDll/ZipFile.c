...
static int readzipfile1( struct Globals *pG, HANDLE hInz );
int readzipfile( struct Globals *pG )
{
  ...
  if ( pG->zipfile != NULL )
  {
    if ( pG->verbose )
    {
      Inform( pG, 0, IDIAG, "ready to open: %s for read only", pG->zipfile );
    }
  }
...
int readzipfile1( struct Globals *pG, HANDLE hInz )
{
  ...
  for ( ;; )
  {
    while ( ReadFile( hInz, b, 1, &( ulg )w, NULL ) && w == 1 && b[0] != 0x50 )
      p++;
    // 0x50 == 'P'
    if ( !ReadFile( hInz, b + 1, 3, &( ulg )w, NULL ) || w != 3 || ( s = LG( b )
        ) == LOCSIG || s == ENDSIG )
      break;
    w =  - 1;
  ...
  while ( LG( b ) == LOCSIG )
  {
    ...
    *x = z;
    ...
    if ( n == 0 )
    {
      Inform( pG, 0, IWARNING, "zero-length name for entry #%d", ( ulg )pG
             ->zcount + 1 );
#ifndef DEBUG
    ...
    if (( flg &8 ) != 0 )
    {
      ...
      if ( s == 0 )
      {
        ...
        if (( sfp != p + 16L ) || w )
        {
          Inform( pG, 0, IWARNING, "bad extended local header for : %s", z
                 ->zname );
...
//               !GetLastError()), ("bad compressed size with extended header"), (0) );
        ...
        if ( LG( b ) != EXTLOCSIG )
        {
          Inform( pG, 0, IWARNING, "extended local header not found for :%s", z
                 ->zname );
    ...
    else if ( pG->fix > 1 )
    {
      ...
      if ( s != z->siz )
      {
        Inform( pG, 0, 0, " compressed size %ld, actual size %ld for %s\n", z
               ->siz, s, z->zname );
        z->siz = s;
      }
  ...
  if ( pG->fix )
  {
    ...
    {
      Inform( pG, 0, 0, "zip warning: %s %s truncated.\n", pG->zipfile, pG->fix
             > 1 ? "has been" : "would be" );
      if ( pG->fix == 1 )
      {
  ...
  else
  {
    while ( LG( b ) == CENSIG )
    {
      if ( z == NULL )
      {
        Inform( pG, 0, IWARNING, "extraneous central header signature" );
      ...
      for ( m = 0, u = ( char* )z, n = 0; n < LOCHEAD - 2; n++ )
      {
        if ( u[n] != b[n + 2] )
        {
          if ( !m && pG->noisy )
          {
            Inform( pG, 0, IWARNING, "local and central headers differ for :%s",
                   z->zname );
          }
          ...
          if ( pG->noisy )
          {
            Inform( pG, 0, IWARNING, " offset %d--local = %02x, central = %02x",
      ...
      {
        FREE( t );
        Inform( pG, 0, IWARNING, "names in local and central differ for :%s", z
               ->zname );
      ...
      if ( pG->verbose )
      {
        ...
        {
          Inform( pG, 0, IWARNING, "%s needs unzip %d.%d on system type %d: ",
        ...
        if ( z->how > DEFLATE )
        {
          Inform( pG, 0, IWARNING, "unknown compression method %u: :%s", z->how,
                 z->zname );
        }

        if ( z->dsk )
        {
          Inform( pG, 0, IWARNING, "starts on disk %u: %s", z->dsk, z->zname );
        }

        if ( z->att &~1 )
        {
          Inform( pG, 0, IWARNING, "unknown internal attributes = 0x%04x: %s",
                 z->att, z->zname );
        }
      }
  ...
  if ( !pG->fix )
  {
    ...
    if ( SH( ENDDSK + b ) || SH( ENDBEG + b ) || SH( ENDSUB + b ) != SH( ENDTOT
        + b ))
      Inform( pG, 0, IWARNING, "multiple disk information ignored" );
    if ( pG->zcount != ( extent )( SH( ENDSUB + b )))
    {
      Inform( pG, 0, IWARNING, "count in end of central directory incorrect" );
    ...
    // && !pG->adjust )
    {
      if ( !pG->adjust )
      {
        Inform( pG, 0, IWARNING, "central directory start is incorrect", z
               ->zname );
  ...
  if ( pG->zipbeg && pG->verbose )
  {
    ...
    Inform( pG, 0, IWARNING, "%s has a preamble of %lu bytes", pG->zipfile, pG
           ->zipbeg );
  }

  if ( !pG->fix && ReadFile( hInz, b, 1, &( ulg )w, NULL ) && w == 1 )
    Inform( pG, 0, IWARNING, "garbage at end of zip file ignored" );
...
//   to.
int putlocal( struct zlist *z, HANDLE f, struct Globals *pG )
{
  unsigned long k;
  struct ZipLocalHeader zlh;
  if ( pG->verbose < 0 )
  {
    Inform( pG, 0, IDIAG, "writing Local Header: %s", z->name );
  }
...
//   :: File to write to.
int putextended( struct zlist *z, HANDLE f, struct Globals *pG )
{
  unsigned long k;
...
//   to write to.
int putcentral( struct zlist *z, HANDLE f, struct Globals *pG )
{
  unsigned long k;
...
putend( int n, ulg s, ulg c, HANDLE f, struct Globals *pG )
{
  //                        pG->zcomlen, pG->zcomment, pG->hOutz )
  unsigned long k;
...
int zipcopy( struct zlist *z, HANDLE x, HANDLE y, struct Globals *pG )
{
  // added R. Aelbrecht for OEM2ISO conversion
  int hostnum;
  int hostver;
  ...
  user_callback( zacItem, 0, n /*z->siz*/, z->name, pG );
  // end add
...
int EraseFile( struct Globals *pG, char *Fname )
{
  ...
  SHF.hwnd = pG->global_handle; // 1.75
  SHF.wFunc = FO_DELETE;
  SHF.pFrom = Fname;
  SHF.pTo = NULL;
  ...
  for ( z = pG->zfiles; z != NULL; z = z->nxt )
  {
    if ( z->mark == 1 || z->trash )
    {
      ...
      if ( z->zname[z->nam - 1] != '\\' )
      {
        ...
        if ( pG->verbose )
          Inform( pG, 0, 0, "deleting file %s", z->name );
        if ( EraseFile( pG, GetFullPath( pG, z->name )))
        {
          // v1.6017
          // if ( destroy( z->name ) ) {
          // zipwarn ( "error deleting", z->name );
          Inform( pG, 0, IWARNING, "error deleting %s", z->name );
        }
  ...
  if ( n )
  {
    ...
    for ( i = 0; i < n; i++ )
    {
      ...
      if ( i == 0 || strcmp( s[i]->zname, s[i - 1]->zname ) != 0 )
      {
        if ( pG->verbose )
        {
          Inform( pG, 0, 0, "trashing directory %s (if empty)", s[i]->name );
        }
