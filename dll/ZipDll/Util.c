...
// return uppercase of character char __fastcall UpCase( char c) { if
//   (c>='a' &
//   &
//   c<='z') return c-('a'-'A'); if (c=='/') return '\\'; return c; }
...
dosmatch( const char *p, const char *s )
{
  ...
  {           // no period - no part 2
    pe = p2e; // end
    if ( (pe > ( uch * )p) && (*(pe - 1) == '*') )  // ended in '*' - match
                                                              ///any/no s2
      return pat_match( p, s, pe, se );
    else
      return s2 ? 0 : pat_match( p, s, pe, se );    // s2 must be empty
  }

  // p==>s , p2==>s2 , (.e=end section)
  if ( pat_match( p2, s2, p2e, s2e ) && pat_match( p, s, pe, se ) )
    return 1;
  return 0;
}
...
// void init_upper ( struct Globals *pG ) { int c; for ( c = 0; c < sizeof (
//   pG->upper ); c++ ) pG->upper[ c ] = ( uch ) c; for ( c = 'a'; c <= 'z';
//   c++ ) pG->upper[ c ] = ( uch ) ( c - 'a' + 'A' ); }
