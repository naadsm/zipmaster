...
static void partial_clear ( struct Globals *pG );

#ifdef DEBUG
#  define OUTDBG(c) \
   if ((c) < 32 || (c) >= 127) DLLprintf( "\\x%02x", (c) ); else putc( (c), stderr );
#else
#  define OUTDBG( c )
#endif
...
#       endif
    ...
    do
    {
        ...
        {    /* possible to have consecutive escapes? */
            ...
            {
                ++codesize;
                Trace ( ( pG, " (codesize now %d bits)\n", codesize ) );
            }
            else
                if ( code == 2 )
                {
                    Trace ( ( pG, " (partial clear code)\n" ) );
                    partial_clear ( pG );     /* clear leafs (nodes with no children) */
                    Trace ( ( pG, " (done with partial clear)\n" ) );
        ...
        if ( parent[ curcode ]  == FREE_CODE )
        {
            ...
            KwKwK = true;
            Trace ( ( pG, " (found a KwKwK code %d; oldcode = %d)\n", code, oldcode ) );
         ...
         *-----------------------------------------------------------------------*/
        Trace ( ( pG, "code %4d; oldcode %4d; char %3d (%c); string [", code,
                                                   ...
                                                   || *newstr >= 127 )  ? ' ' : *newstr ) );
        {
            ...
            for ( p = newstr; p < newstr + len; ++p )
            {
                *pG->outptr++ = *p;
                ...
                {
                    Trace ( ( pG, "doing flush(), outcnt = %lu\n", pG->outcnt ) );
                    if ( ( error = flush ( pG, pG->realbuf, pG->outcnt, true ) )  != 0 )
                        DLLprintf ( "unshrink:  flush() error (%d)\n", error );
                    Trace ( ( pG, "done with flush()\n" ) );
                    pG->outptr = pG->realbuf;
                    pG->outcnt = 0L;
                }
            }
    ...
    if ( pG->outcnt > 0L )
    {
        Trace ( ( pG, "doing final flush(), outcnt = %lu\n", pG->outcnt ) );
        if ( ( error = flush ( pG, pG->realbuf, pG->outcnt, true ) )  != 0 )
            DLLprintf ( "unshrink:  flush() error (%d)\n", error );
        Trace ( ( pG, "done with flush()\n" ) );
    }
 ...
 */
static void
partial_clear ( struct Globals *pG )
{
