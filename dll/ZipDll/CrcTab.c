 ...
 */
/* $Id: CrcTab.c,v 1.1 2006-02-27 19:14:38 areeves Exp $ */
/*
...
static ulg *crc_table = NULL;
    #if 0         // not used, since sizeof("near *") <= sizeof(int)
// Use this section when access to a "static int" is faster than access to a "static pointer" (e.g.: i86 16bit code with far pointers). static int  crc_table_empty = 1;
      #define CRC_TABLE_IS_EMPTY  ( crc_table_empty != 0 )
      #define MARK_CRCTAB_FILLED  crc_table_empty = 0
    #else
// Use this section on systems where the size of pointers and ints is equal (e.g.: all 32bit systems).
      #define CRC_TABLE_IS_EMPTY  ( crc_table == NULL )
      #define MARK_CRCTAB_FILLED  crc_table = crctab_p
    #endif
  #else // !DYNALLOC_CRCTAB
static ulg crc_table[256];
...
static void make_crc_table()
{
  ...
  //   used in the creation of the crc_table values can be recomputed. For
  //   production versions of this function, it is more efficient to supply the
  //   resultant pattern at compile time.
  ...
  #endif
  #ifdef DYNALLOC_CRCTAB
  crctab_p = ( ulg* )MALLOC( 256 *sizeof( ulg ));
  if ( crctab_p == NULL )
    ziperr( ZEN_MEM01 );
  #endif // DYNALLOC_CRCTAB
  for ( n = 0; n < 256; n++ )
