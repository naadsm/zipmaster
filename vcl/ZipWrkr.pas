...
(* TZipWorker VCL by Chris Vleghert and Eric W. Engler
 ...
 v1.78  by Russell Peters March  18, 2004.
...
const
  ...
  ZIPMASTERBUILD: String = '1.78.3.0';
...
{$ENDIF}
    ...
    function Load_Zip_Dll(silent: boolean): Integer;
    function Load_Unz_Dll(silent: boolean): Integer;
...
1.78 20 Nov 2004 returns version if ok
*)

function TZipWorker.Load_Zip_Dll(silent: boolean): Integer;
begin
  try
    Result := FZipDll.LoadDll(ZIPVERSION, True);
  except
...
function TZipWorker.Load_Unz_Dll(silent: boolean): Integer;
begin
  try
...
(*? TZipWorker.OpenEOC
1.78.2.2 24 January 2005 RP fix Overflow and Range errors
...
1.78 27 Nov 2004 - fix answer no 
...
var
  ...
  begin
    ...
    if (Res = 0) or (Res = idCancel) //or (Res = idNo) then
      or ((Res = idNo) and FDriveFixed) then
      raise EZipMaster.CreateResDisp(DS_Canceled, False);
    ...
    begin
      ...
      case ActionCode of
        ...
        begin
          ...
          begin
            OrigName    := SetSlash(PChar(ZCallBackRec^.Data),
              psdExternal);
            OldFileName := Msg;
            IsChanged   := False;
...
//              Msg + ' ' + IntToStr((FileSize and $FF) - 1));
            OnMessage(Master, GE_Unknown,
              ZipFmtLoadStr(GE_Skipped, [Msg,(FileSize and $FF) - 1]));
        end;
...
begin
  ...
  try
    ...
    if FileSeek64(FInFileHandle, 0, 0) = -1 then
      raise EZipMaster.CreateResDisp(DS_FailedSeek, True);
    ...
    if FileSeek64(FInFileHandle, EOC.CentralOffset, 0) = -1 then
      raise EZipMaster.CreateResDisp(DS_FailedSeek, True);
    ...
    begin
      ...
      MDZDp^.ComprSize  := CEH.ComprSize;   // RP may not be valid in local header
//      MDZDp^.UnComprSize := CEH.UnComprSize;
      ...
      // Seek past the extra field and the file comment.
      if FileSeek64(FInFileHandle, CEH.ExtraLen + CEH.FileComLen, 1)
          = -1 then
        raise EZipMaster.CreateResDisp(DS_FailedSeek, True);
    end;
    ...
    begin
      ...
      MDZDp^.RelOffLocal := FileSeek64(FOutFileHandle, 0, 1);
      ...
      // RP use uncompressed size from central header
      if CopyBuffer(FInFileHandle, FOutFileHandle, {LOH}MDZDp^.ComprSize) <> 0 then
    ...
    begin
      ...
      // Save the first Central directory offset for use in EOC record.
      if i = 0 then
        EOC.CentralOffset := Cardinal(FileSeek64(FOutFileHandle, 0, 1));
  ...
  try
    ...
    begin
      ...
      if Assigned(OnMessage) then
        OnMessage(Master, ErrCode, E.Message);
      Result      := -9;
      FSuccessCnt := 0;
    end;
    on E: Exception do
