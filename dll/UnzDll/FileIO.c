...
static int disk_error( struct Globals * pG );

//#ifdef USE_STRM_OUTPUT
static WriteToMemory( struct Globals * pG, uch * rawbuf, ulg size );
...
int open_input_file( struct Globals * pG )
{
  Trace( ( pG, "open_input_file; fname=%s", pG->zipfn ) );
  ...
  Trace( ( pG, "zip file %s opened OK", pG->zipfn ) );
  return 0;
}

void ShowSysMsg( struct Globals * pG, DWORD Error )
{
  LPVOID lpMsgBuf;
...
* Return 1 if failed. */
int open_outfile( struct Globals * pG )
{
#       ifdef USE_STRM_OUTPUT
  if ( pG->redirect_data )
...
#       endif
  ...
  {
    Trace( ( pG, "open_outfile:  stat(%s) returns file exists\n", pG->filename ) );
    if ( !( pG->statbuf.st_mode & S_IWRITE ) )
    {
      Trace( ( pG, "open_outfile:  existing file %s is read-only\n", pG->filename ) );
      chmod( pG->filename, S_IREAD | S_IWRITE );
    }
    if ( !DeleteFile( pG->filename ) )
    {
      Trace( ( pG, "DeleteFile( %s ) failed:", pG->filename ) );
      ShowSysMsg( pG, GetLastError() );
      return 1;
    }
    SHChangeNotify( SHCNE_DELETE, SHCNF_PATH, GetFullPath( pG, pG->filename ), NULL );
    if ( pG->vflag )
      DLLprintf( "Deleted pre-existing file %s", pG->filename );
  }
#  ifdef NOVELL_BUG_FAILSAFE
  if ( pG->dne && ( ( pG->outfile = fopen( pG->filename, FOPR ) ) != NULL ) )
  {

    DLLprintf( "File %s open failed due to Novell Bug", pG->filename );
    fclose( pG->outfile );
    return 1;
...
#  endif
  ...
  {
    DLLprintf( "Error opening file %s for write", pG->filename );
    return 1;
  }
  //EWE SHChangeNotify( SHCNE_CREATE, SHCNF_PATH, GetFullPath( pG, pG->filename ), NULL );
  Trace( ( pG, "open_outfile:  fopen(%s) for writing succeeded\n", pG->filename ) );
...
void undefer_input( struct Globals * pG )
{
  if ( pG->incnt > 0 )
    pG->csize += pG->incnt;
...
void defer_leftover_input( struct Globals * pG )
{
...
unsigned readbuf( struct Globals * pG, char * buf, register unsigned size )
{
  ...
  while ( size )
  {
    if ( pG->incnt <= 0 )
    {
      ...
      {
        DLLprintf( "Error reading ZIP file" );
        return 0;
...
int readbyte( struct Globals * pG )
{
  if ( pG->mem_mode )
    return EOF;
  if ( pG->csize <= 0 )
  ...
  if ( pG->incnt <= 0 )
  {
    ...
    else if ( pG->incnt < 0 )
    {
      ...
      DLLprintf( "Error reading from ZIP file" );
      EXIT( UEN_READ01 );
...
int flush( struct Globals * pG, uch * rawbuf, ulg size, int unshrink )
{
  register uch * p, * q;
  uch * transbuf;
  //      ulg           crcwas;
...
#       ifdef USE_STRM_OUTPUT
  ...
  if ( ProgrSize )
    user_callback( pG, 2, 0, ProgrSize, NULL ); // bump up progress bar
#  else

  if ( size )
    user_callback( pG, 2, 0, size, NULL );
#       endif
...
time_t dos_to_unix_time( unsigned ddate, unsigned dtime )
{
  ...
  TTrace( ( pG, "dos_to_unix_time:\n" ) );
  TTrace( ( pG, "  m_time before timezone = %ld\n", ( long )m_time ) );
...
int check_for_newer( struct Globals * pG, char * filename )
{
  ...
  //Trace( (pG, "check_for_newer:  doing stat(%s)\n", filename) );
  if ( stat( filename, & pG->statbuf ) )
  {
    Trace( ( pG, "check_for_newer:  stat(%s) returns %d:  file does not exist\n", filename,
         stat( filename, & pG->statbuf ) ) );
    return DOES_NOT_EXIST;
  }
  //Trace( (pG, "check_for_newer:  stat(%s) returns 0:  file exists\n", filename) );
  ...
  * the OS specific code from the "unix/" subdirectory. */
  ...
  {
    TTrace( ( pG, "check_for_newer:  using Unix extra field mtime\n" ) );
  ...
  Trace( ( pG, "check_for_newer:  existing %ld, archive %ld, Result %d\n", existing, archive, existing >= archive ) );
...
int do_string( struct Globals * pG, unsigned int len, int option )
{
  ...
  switch ( option )
  {
    ...
    * are available (we should be either before or in between any file processing). */
    ...
    case DS_FN:
      ...
      if ( len >= FILNAMSIZ )
      {
        DLLprintf( "Warning - filename too long; truncated" );
        error = PK_WARN;
      ...
      * ASCII" into the compiler's (system's) internal text code page */
      ...
      if ( pG->pInfo->vollabel && len > 8 && pG->filename[8] == '.' )
      {
...
void ExtASCII2Native( struct Globals * pG, char * string, int hostnum, short int hostver )
{
...
char * GetFullPath( struct Globals * pG, char * Filename )
{
  ...
  * p = 0; // in case both ExtractDir && Filename were empty
  ...
  char fn[MAX_PATH], * p1 = fn;
  DWORD len;

  lstrcpy( fn, Filename );
  for ( ; p1 < fn + lstrlen( fn ); p1++ )
    if ( * p1 == '/' )
      * p1 = '\\';

  //len = GetCurrentDirectory( 2048, pG->ewemsg );
  lstrcpy( pG->ewemsg, pG->ExtractDir ); // v1.6024
  len = lstrlen( pG->ExtractDir );
  if ( pG->ewemsg[len - 1] != '\\' )
  { // v1.6026
    pG->ewemsg[len] = '\\';
    pG->ewemsg[len + 1] = 0;
  }
  return lstrcat( pG->ewemsg, fn );
#endif
}

#ifdef USE_STRM_OUTPUT
int WriteToMemory( struct Globals * pG, uch * rawbuf, ulg size )
{
...
long vclSeek( struct Globals * pG, long Offset, int FromWhere )
{
  if ( pG->UseInStream )
  {
    switch ( FromWhere )
...
int vclRead( struct Globals * pG, void * buf, unsigned LenToRead )
{
  if ( pG->UseInStream )
  {
    if ( pG->StreamPos == pG->InStreamSize )
