...
//    ush chdrseq;                  /* v1.6023 added   */
    ...
    unsigned chdrseq:20;          // uses unused bits
}
min_info;
 ...
 *---------------------------------------------------------------------------*/
int unzip ( struct Globals *pG, int argc, char **argv );
int uz_opts ( struct Globals *pG, int *pargc, char ***pargv );
int usage ( struct Globals *pG, int error );

/*---------------------------------------------------------------------------
 *  Functions in process.c (main driver routines):
 *---------------------------------------------------------------------------*/
void process_zipfiles ( struct Globals *pG );
int uz_end_central ( struct Globals *pG );
int process_cdir_file_hdr ( struct Globals *pG );
int get_cdir_ent ( struct Globals *pG );
int process_local_file_hdr ( struct Globals *pG );
 ...
 *---------------------------------------------------------------------------*/
int zi_opts ( struct Globals *pG, int *pargc, char ***pargv );
int zi_end_central ( struct Globals *pG );
int zipinfo ( struct Globals *pG );

/*---------------------------------------------------------------------------
 *  Functions in list.c (generic zipfile-listing routines):
 *---------------------------------------------------------------------------*/
//int      list_files             ( struct Globals *pG );
//int      ratio                  ( ulg uc, ulg c );
//void     fnprint                ( struct Globals *pG );

/*---------------------------------------------------------------------------
 *  Functions in fileio.c:
 *---------------------------------------------------------------------------*/
int open_input_file ( struct Globals *pG );
int open_outfile ( struct Globals *pG );  /* also vms.c */
void undefer_input ( struct Globals *pG );
void defer_leftover_input ( struct Globals *pG );
unsigned readbuf ( struct Globals *pG, char *buf, register unsigned len );
int readbyte ( struct Globals *pG );
int fillinbuf ( struct Globals *pG );
int flush ( struct Globals *pG, uch * buf, ulg size, int unshrink );
void handler ( int signal );
time_t dos_to_unix_time ( unsigned ddate, unsigned dtime );
int check_for_newer ( struct Globals *pG, char *filename );       /* os2,vmcms,vms */
 ...
 *---------------------------------------------------------------------------*/
int explode ( struct Globals *pG );       /* explode.c  */
int huft_free ( struct huft *t ); /* inflate.c  */
int huft_build ( struct Globals *pG, unsigned *b, unsigned n,
                 unsigned s, const ush * d, const ush * e, struct huft **t,
                 int *m );
int inflate ( struct Globals *pG );       /* inflate.c  */
int inflate_free ( struct Globals *pG );  /* inflate.c  */
void unreduce ( struct Globals *pG );     /* unreduce.c */
int unshrink ( struct Globals *pG );      /* unshrink.c */
 ...
 *---------------------------------------------------------------------------*/
struct Globals *InitGlobals ( void );     /* globals.c */
void envargs ( struct Globals *pG, int *Pargc, char ***Pargv, char *envstr, char *envstr2 );      /* envargs.c */
void mksargs ( int *argcp, char ***argvp );       /* envargs.c */
int match ( char *s, char *p );
int iswild ( char *p );           /* match.c   */
ulg *get_crc_table ( void );      /* crctab.c  */
ulg crc32 ( ulg crc, const uch * buf, extent len );       /* crc32.c */
int dateformat ( void );          /* currently, only msdos.c */
void version ( struct Globals *pG );      /* local */
int mapattr ( struct Globals *pG );       /* local */
int mapname ( struct Globals *pG, int renamed );  /* local */
int checkdir ( struct Globals *pG, char *pathcomp, int flag );    /* local */
char *do_wild ( struct Globals *pG, char *wildzipfn );    /* local */
char *GetLoadPath ( struct Globals *pG ); /* local */
void close_outfile ( struct Globals *pG );        /* local */
...
#define READBITS( nbits, zdest ) { if( nbits>pG->bits_left ) {int temp; pG->zipeof = 1; \
  while (pG->bits_left <= 8 *(sizeof( pG->bitbuf )- 1) && (temp = NEXTBYTE) != EOF) { \
...
#define cpISO  1
#define cpOEM  2
extern void ExtASCII2Native ( struct Globals *pG, char *string, int hostnum,
                                  short int hostver );
...
extern const char *fnames[ 2 ];   /* In Globals.c */
